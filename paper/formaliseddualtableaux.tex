...
\begin{abstract}
  ...
  familiar with dual-tableaux  who might be daunted by the idea of
...
tradition by wishing Ewa a happy 60th birthday.
...
calculi. Specifically, we show how to formalise the dual-tableaux
...
operator for ``member of type''. Thus, \texttt{a : alpha} encodes that
``\texttt{a} is of type \texttt{alpha}''. If also \texttt{b : beta} then \hol{}
...
$x \in P$ (\texttt{x IN P}) means exactly $P\ x$ (\texttt{P x)},
and the type $\alpha\ \mathit{set}$ 
is the same as $\alpha \to \mathit{bool}$:
telling us whether a particular term $x$ (say) of type 
$\mathtt{\alpha}$ is or
is not in the set $P$ of type $\alpha\ \mathtt{set}$ as determined by the
...
$\mathit{FST}$ (as a set) is of type 
...
\mbox{\rm
  \begin{tabular}[c]{ll}
  \\ \texttt{get\_ts sfs} & \texttt{: (bool \# 'f) set -> 'f set}
  \\ \texttt{get\_ts sfs} & 
...
\cite[Figure~6]{fitting-dual-tableau}.
...
Thus \texttt{rule\_sk} (rule-skeleton) takes an item of type
\texttt{'a} (here, a signed formula) and returns a set of conclusions
where each conclusion is a set of items of type \texttt{'a} (here,
signed formula), while \texttt{rule} (an ordinary rule) takes an item
of type \texttt{'a} (here, a set of signed formulae) and returns a set
...
as in~\cite[Fig.\ 3]{fitting-dual-tableau}, takes
a signed formula and gives a set of sets of signed formulae
(so of type \texttt{'sf rule\_sk}),
adding context gives a rule as
in~\cite[Fig.\ 6]{fitting-dual-tableau},
which takes a 
set of signed formulae and gives a set of sets of signed formulae
...
For rules such as the last two of~\cite[Fig.\ 6]{fitting-dual-tableau},
...
$F$-signed context in the conclusion (as in the last two rules of
\cite[Figure~6]{fitting-dual-tableau}),
...
\item[\texttt{ant\_idt\_rule}:] the skeletons of the
  \texttt{imp\_right} and \texttt{not\_right} rules which have only a
   ...
   (see \cite[Figure~6]{fitting-dual-tableau}).
 \end{description}
\end{definition}
...
Notice that we do not stipulate a rule for closing a dual-tableau,
...
\begin{definition}[\texttt{fringe\_rules}]\label{fringe-rules}
...
\begin{verbatim}
! rs s sfr. rs (s, sfr) 
          ==> fringe_rules rs ({s} UNION rf, sfr UNION rf) ;
fringe_rules : 'sfs rule set -> ('sfs set # 'sfs set) set ;
\end{verbatim}
% rules rewritten to avoid use of INSERT 
...
\texttt{fringe\_rule rs} gives the set of resulting transformations
...
use \texttt{fringe\_rule} where \texttt{'sfs} is  the type of sets of signed
...
\begin{verbatim}
fringe_rules idt_tab_rule : ('a sf set set # 'a sf set set) set ; 
\end{verbatim}

At this point we note that in \hol, sets and predicates are identified
and so $x \in P$, (\textit{i.e.} \texttt{x IN P}), means exactly $P\
x$ (\textit{i.e.} \texttt{P x}).
Consequently, 
Definition~\ref{fringe-rules} of \texttt{fringe\_rules} might be more clearly written:
\begin{verbatim}
(s, sfr) IN rs 
           ==> ({s} UNION rf, sfr UNION rf) IN fringe_rules rs
\end{verbatim}
This is an inductive definition which means that \texttt{fringe\_rules rs}
...
\label{br-closes}\label{dt-closes}\label{at-closes}
  A branch tip, ie, a leaf, $\mathtt{sfs}$ is closed if it contains some
  formula $\mathtt{f}$ signed
  $\mathtt{F}$ and $\mathtt{T}$:
\begin{verbatim}
   br_closes : 'a sf set -> bool
   br_closes sfs = ?f. (T, f) IN sfs /\ (F, f) IN sfs 
\end{verbatim}

  A dual-tableau (fringe) $\mathtt{sfss}$ is closed if every 
  branch $\mathtt{sfs}$ in it closes: 
\begin{verbatim}
   dt_closes : 'a sf set set -> bool
   dt_closes sfss = !sfs. sfs IN sfss ==> br_closes sfs 
\end{verbatim}
  ...
  $\mathtt{F}$ and $\mathtt{T}$:
\begin{verbatim}
   at_closes : 'a sf set -> bool
   at_closes sfs = ?p. (T, Atom p) IN sfs /\ (F, Atom p) IN sfs
\end{verbatim}
...
\begin{quote}
  ...
  the initial fringe
  $\{\{(T,f)\}\}$ and $bot$ is closed then the formula
  $f$ is true in every world $w$ of $(R, pv)$:
\begin{verbatim}
Kripke_model R pv ==>
   RTC (CURRY (fringe_rules idt_tab_rule)) {{(T,f)}} bot ==>
   dt_closes bot ==> forces R pv w f
\end{verbatim}
\end{quote}
...
\verb!A /\ B ==> C!, which is why the English prose uses ``and''.
...
$T$-signed in \textit{sfs}, respectively.  According to
...
terms of \texttt{sf\_val\_aux} which is used to define
...
We want to prove soundness in terms of dual-tableaux closing, so we have

\begin{lemma}[\texttt{idt\_br\_sound},\texttt{idt\_dt\_sound}]
\label{idt-br-sound} \label{idt-dt-sound}
  If a dual-tableau branch closes, then the auxiliary valuation (using
  \texttt{sfs\_val}) of the leaf of that branch is true, and if a
  dual-tableau closes, then the valuation (using
  \texttt{tab\_val}) of the fringe is true.
\end{lemma}
\begin{verbatim}
   idt_br_sound : br_closes br ==> sfs_val R pv w br
   idt_dt_sound : dt_closes tab ==> tab_val R pv w tab
\end{verbatim}
...
For the first six rules of \cite[Figure~6]{fitting-dual-tableau}
...
\begin{lemma}[\texttt{idt\_rules\_aux\_eqv}] \label{idt-rules-aux-equiv}
  ...
  \cite[Figure~6]{fitting-dual-tableau}(without the context $S$),
...
Why do we suddenly need reflexivity?  Because for the $\supset$-F
...
for the last two rules of~\cite[Figure~6]{fitting-dual-tableau}.

\begin{lemma}[\texttt{ant\_rules\_eqv}] \label{ant-rules-eqv} For the
  skeletons of the last two rules of \cite[Figure~6]{fitting-dual-tableau}
...
% \begin{verbatim}
% idt_pres_frg :
%   Kripke_model R pv ==> fringe_rules idt_tab_rule (prev, next) 
...
\begin{verbatim}
  Kripke_model R pv  ==> 
    fringe_rules idt_tab_rule (prev, next) ==> 
       tab_val R pv w next  ==> tab_val R pv w prev
\end{verbatim}
...
\begin{verbatim}
 Kripke_model R pv ==>
    !bot. RTC (CURRY (fringe_rules idt_tab_rule)) top bot ==> 
...
If a dual-tableau for the the signed formula $(T,f)$ closes
then any model \textit{R, pv} forces \textit{f} at any world
% If a dual-tableau, starting with the signed formula $(T,f)$, closes,
% then any model \textit{R, pv} forces \textit{f} at any world
\textit{w}.
%idt_sound : 
\begin{verbatim}
...
proof~\cite{fitting-dual-tableau}.

\subsection{$I$-tautologous sets of signed formulae}

We give two ways to formalise $I$-tautologous sets, one folliwing Fitting
...
\cite[Definition~3]{fitting-dual-tableau}, but without the
...
rule application (see \cite[Definition~2.1]{fitting-dual-tableau}).
...
a set \texttt{rs} of rules (i.e.\ \texttt{Itautss rs top} holds) if
starting with \texttt{top} and repeatedly applying rules from
\texttt{rs} gives
a fringe \texttt{bot} which closes. A set \texttt{s} of signed
...
Itautss rs top = 
  ?bot. RTC (CURRY (fringe_rules rs)) top bot /\ dt_closes bot
...
%   ?bot. RTC (CURRY (fringe_rules rs)) s bot /\ dt_closes bot` ;
\end{definition}
...
finite, which captures that each rule is finitely branching as shown next.
...
\begin{definition}[\texttt{Itauti}]\label{Itauti} 
  ...
  \begin{enumerate} 
  \item \label{Itauti-1}
    \texttt{top} is itself closed w.r.t.\ \texttt{rs}, or
  \item \label{Itauti-2}
...
\begin{verbatim} 
(!top. br_closes top ==> Itauti rs top) /\ 
(!top. (?rb. (top,rb) IN rs /\ !br. br IN rb ==> Itauti rs br)  
                     ==> Itauti rs top)
\end{verbatim}
\end{definition}

First, note that the linguistic ``or'' between clauses~\ref{Itauti-1}
...
of \texttt{Itauti rs}: \textit{i.e.} \texttt{Itauti rs} is the smallest set
...
finite.  If such a dual-tableau closes then it meets the definition of
...
atom $a$).
Fitting \cite{fitting-dual-tableau} uses atomic closure.
We have used closure without this restriction, but we 
...
(thus \texttt{Itauti} = \texttt{Itautg br\_closes}).
...
Then \texttt{Itautg at\_closes idt\_tab\_rule} is the set of
...
A set is $I$-tautologous (per Definitions \ref{Itauti} and \ref{br-closes})
iff it is $I$-tautologous (defined to require atomic closure)
\end{lemma}
\begin{verbatim}
Itauti idt_tab_rule sfs <=> Itautg at_closes idt_tab_rule sfs 
\end{verbatim}
...
\item[\rm (ii) Atomic closure:] We dropped this assumption and allowed
  ...
  which closes using non-atomic closure can be extended to a
  dual-tableau which closes atomically.
\item[\rm (iii) Single use restriction:] This restriction is also
  ...
  stated by Fitting~\cite[Definition~2.1]{fitting-dual-tableau}).
\end{description}

\subsection{The Lindenbaum construction}

We now discuss proving Theorem 2.6 (``after
Lindenbaum'')~\cite{fitting-dual-tableau}.  Fitting assumes that the
...
Here, \texttt{PSUBSET} captures $s \subset t$.
...
sets~\cite[page~17]{fitting-dual-tableau}. We therefore define a new type
...
\begin{lemma}[\texttt{NON\_ITAUT\_RULE}]\label{NON-ITAUT-RULE}
  ...
  and \texttt{s} is maximal non-$I$-tautologous wrt.\ \texttt{rs},
...
%
The following result corresponds to the text following
\cite[Thm 2.6]{fitting-dual-tableau}.
...
% \begin{verbatim}
%    ?bot. RTC (CURRY (fringe_rules idt_tab_rule)) {{(T,f)}} bot 
%          /\ dt_closes bot
% \end{verbatim}
...
\cite[Definition~3.3]{fitting-dual-tableau}. 
...
the inductive definition. One caveat: Jeremy Dawson has over 20 years
...
theorem proving is time-consuming and laborious! All of our \hol-code
